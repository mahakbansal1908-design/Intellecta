...
def record_outcome(
    *,
    ...
    descriptor, which is structured from the tool call metadata."""
    arg_words: list[str] = []
...
def clear() -> None:
    ...
    try:
        ...
        print(f"[memory] Warning unlinking STATE_PATH: {e}")
        
    # Clear all files in the state/artifacts cache directory
    try:
        ...
        print(f"[memory] Warning cleaning artifacts directory: {e}")
                
    try:
        VectorIndex(STATE_PATH.parent).clear()
    except Exception as e:
        print(f"[memory] Warning clearing VectorIndex: {e}")
