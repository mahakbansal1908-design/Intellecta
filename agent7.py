...
async def run(query: str, on_log: Optional[Callable[[str], Awaitable[None]]] = None) -> str:
    ...
    await log(f"\n{'═' * 78}")
    await log(f"run {run_id}  ─  query: {query}")
    await log(f"{'═' * 78}")

    try:
        memory.remember(query, source="user_query", run_id=run_id)
    except Exception as e:
        await log(f"[memory.remember] skipped: {e}")
    ...
    async with stdio_client(server_params) as (read, write):
        async with ClientSession(read, write) as session:
            try:
                ...
                raise RuntimeError("MCP server connection timed out on startup. Verify mcp_server.py runs without crashes.")
            mcp_tools = (await session.list_tools()).tools
            tools_for_decision = _mcp_tools_for_decision(mcp_tools)
            await log(f"[mcp] loaded {len(mcp_tools)} tools: {[t.name for t in mcp_tools]}")
            ...
            for it in range(1, MAX_ITERATIONS + 1):
                ...
                await log(f"[memory.read]   {len(hits)} hits")
                ...
                for g in obs.goals:
                    ...
                    await log(f"[perception]    {flag} {g.id} — {g.text}{attach}")
                ...
                if goal.attach_artifact_id and artifacts.exists(goal.attach_artifact_id):
                    ...
                    await log(f"[attach]        {goal.attach_artifact_id} ({len(blob)} bytes)")

                # 3. DECISION
                out = decision.next_step(goal, hits, attached, history, tools_for_decision)

                if out.is_answer:
                    await log(f"[decision]      ANSWER: {out.answer[:200]}{'...' if len(out.answer) > 200 else ''}")
                    history.append({
                        "iter": it,
                        "kind": "answer",
                        "goal_id": goal.id,
                ...
                # 4. ACTION
                tc = out.tool_call
                await log(f"[decision]      TOOL_CALL: {tc.name}({json.dumps(tc.arguments)[:120]})")
                result_text, art_id = await action.execute(session, tc)
                preview = result_text[:200].replace("\n", " ")
                await log(f"[action]        → {preview}{'...' if len(result_text) > 200 else ''}"
    ...
    await log(f"\n{'═' * 78}")
    await log(f"FINAL: {final_answer}")
    await log(f"{'═' * 78}\n")
    return final_answer
